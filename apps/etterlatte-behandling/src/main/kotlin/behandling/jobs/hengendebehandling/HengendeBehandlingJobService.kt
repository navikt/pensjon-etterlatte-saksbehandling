...
class HengendeBehandlingJobService(
    ...
    private fun run() {
        ...
        val hengendeBehandlinger = inTransaction { dao.hentBehandlingerHengendeIAttestert(sistEndretFoer) }

        if (hengendeBehandlinger.isNotEmpty()) {
            logger.error(
                "Fant ${hengendeBehandlinger.size} behandling(er) som har stått fast i status ATTESTERT i mer enn " +
                    "$grenseForHengende uten endring, dette kan tyde på at iverksettingen har stanset opp: " +
