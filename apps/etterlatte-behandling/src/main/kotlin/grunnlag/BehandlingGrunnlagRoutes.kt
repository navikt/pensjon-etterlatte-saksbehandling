    ...
    route("/behandling/{$BEHANDLINGID_CALL_PARAMETER}") {
        get {
            val opplysningsgrunnlag =
                grunnlagService.hentOpplysningsgrunnlag(behandlingId, brukerTokenInfo)
        ...
        get("{opplysningType}") {
            ...
            val grunnlag = grunnlagService.hentGrunnlagAvType(behandlingId, opplysningstype, brukerTokenInfo)
        ...
        get("revurdering/${Opplysningstype.HISTORISK_FORELDREANSVAR}") {
            val historisk =
                grunnlagService.hentHistoriskForeldreansvar(behandlingId, brukerTokenInfo)
                    ?: throw GenerellIkkeFunnetException()
        ...
        post("/laas") {
            grunnlagService.laasVersjonForBehandling(behandlingId, brukerTokenInfo)
            call.respond(HttpStatusCode.OK)
        }
        ...
        post("/opprett-grunnlag") {
            ...
            grunnlagService.opprettGrunnlag(behandlingId, opplysningsbehov, brukerTokenInfo)
            call.respond(HttpStatusCode.OK)
        }

        post("oppdater-grunnlag") {
            val request = call.receive<OppdaterGrunnlagRequest>()
            grunnlagService.oppdaterGrunnlag(behandlingId, request.sakId, request.sakType, brukerTokenInfo)
            call.respond(HttpStatusCode.OK)
        }

        get("opplysning/persongalleri-samsvar") {
            val persongalleri = grunnlagService.hentPersongalleriSamsvar(behandlingId, brukerTokenInfo)
        ...
        get("personopplysninger") {
            ...
            val grunnlagsopplysninger =
                grunnlagService.hentPersonopplysninger(behandlingId, sakstype, brukerTokenInfo)
