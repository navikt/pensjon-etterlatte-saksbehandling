...
class OppgaveDao(private val datasource: DataSource) {
    ...
    fun finnOppgaverForRoller(roller: List<Rolle>): List<Oppgave>{
        val aktuelleStatuser = roller.flatMap { when(it){
            Rolle.SAKSBEHANDLER -> listOf(BehandlingStatus.UNDER_BEHANDLING, BehandlingStatus.GYLDIG_SOEKNAD, BehandlingStatus.RETURNERT)
        ...
        datasource.connection.use {
            ...
            return stmt.executeQuery().toList {
                val mottattDato = getTimestamp("soekand_mottatt_dato")?.toLocalDateTime()?.atZone(ZoneId.of("UTC"))
                    ...
                    ?: throw IllegalStateException("Vi har en behandling som hverken har soekand mottatt dato eller behandling opprettet dato ")
