...
internal class RealManueltOpphoerServiceTest {
    ...
    fun `skal hente manuelt opphoer`() {
        ...
        val behandlingDaoMock =
            mockk<BehandlingDao> {
                every { hentBehandling(id) } returns
                    manueltOpphoer(
                        sakId = sakId,
                    )
            }
    ...
    fun `skal opprette et manuelt opphoer`() {
        ...
        val behandlingDaoMock =
            mockk<BehandlingDao> {
                every { alleBehandlingerISak(capture(alleBehandlingerISakSlot)) } returns
                    listOf(
    ...
    fun `manuelt opphør får tidligste virkningstidspunkt fra iverksatte behandlinger på saken`() {
        ...
        val behandlingDaoMock =
            mockk<BehandlingDao> {
                every { alleBehandlingerISak(any()) } returns
                    listOf(
                        foerstegangsbehandling(
                            sakId = sakId,
                            status = BehandlingStatus.IVERKSATT,
    ...
    fun `skal ikke kunne opphoere en sak som allerede er manuelt opphoert`() {
        ...
        val alleBehandlingerISak = slot<Long>()
        val behandlingDaoMock =
            mockk<BehandlingDao> {
                every { alleBehandlingerISak(capture(alleBehandlingerISak)) } returns
                    listOf(
    ...
    fun `skal ikke kunne opphøre en sak som ikke har noen iverksatte behandlinger`() {
        ...
        val behandlingDaoMock =
            mockk<BehandlingDao> {
                every { alleBehandlingerISak(sak) } returns
                    listOf(
                        foerstegangsbehandling(
                            sakId = sak,
    ...
    fun `hentManueltOpphoerOgAlleIverksatteBehandlingerISak tar også med andre iverksatte behandlinger på saken`() {
        ...
        val behandlingDaoMock =
            mockk<BehandlingDao> {
                every { alleBehandlingerISak(sakId) } returns
                    listOf(
                        opphoer,
