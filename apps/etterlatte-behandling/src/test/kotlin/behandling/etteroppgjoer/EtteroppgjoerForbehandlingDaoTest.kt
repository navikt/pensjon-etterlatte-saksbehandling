...
class EtteroppgjoerForbehandlingDaoTest(
    ...
    private var etteroppgjoersAar = 2024
    ...
    fun `lagre og oppdatere forbehandling`() {
        ...
        val ny =
            EtteroppgjoerForbehandling(
                ...
                sak = sak,
                brevId = null,
                innvilgetPeriode = Periode(YearMonth.of(etteroppgjoersAar, 1), YearMonth.of(etteroppgjoersAar, 12)),
    ...
    fun `hent forbehandlinger`() {
        ...
        with(etteroppgjoerForbehandlingDao.hentForbehandlingerForSak(sak.id, etteroppgjoersAar)) {
            size shouldBe 2
            forEach {
    ...
    fun `kan lagre ned og hente ut summerte inntekter for forbehandling`() {
                    ...
                    data =
                        listOf(
                            inntektsinformasjonDto(
                                maaned = YearMonth.of(etteroppgjoersAar, Month.JANUARY),
                                inntektListe =
                                    listOf(
        ...
        val loenn = InntektskomponentBeregning.beregnInntekt(InntektBulkResponsDto("a", emptyList()), etteroppgjoersAar)
        val oms = InntektskomponentBeregning.beregnInntekt(InntektBulkResponsDto("a", emptyList()), etteroppgjoersAar)
    ...
    fun `kopier summerte inntekter til ny forbehandling`() {
        ...
        etteroppgjoerForbehandlingDao.lagreSummerteInntekter(
            forbehandlingId,
            SummerteInntekterAOrdningen(
                afp = InntektSummert("A", tolvMndInntekter(etteroppgjoersAar, 5000.toBigDecimal())),
                loenn = InntektSummert("B", tolvMndInntekter(etteroppgjoersAar, 6000.toBigDecimal())),
                oms = InntektSummert("C", tolvMndInntekter(etteroppgjoersAar, 7000.toBigDecimal())),
