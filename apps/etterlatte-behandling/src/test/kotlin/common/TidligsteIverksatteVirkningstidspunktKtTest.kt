...
class TidligsteIverksatteVirkningstidspunktKtTest {
    ...
    fun `ser kun på iverksatte saker`() {
        val behandlingAvbrutt = mockBehandlingMedStatusOgVirkdato(
            BehandlingStatus.AVBRUTT,
            YearMonth.of(2022, Month.AUGUST)
        )
        val behandlingIverksatt = mockBehandlingMedStatusOgVirkdato(
    ...
    fun `tar med tidligste hvis det er flere iverksatte`() {
        val behandlingTidlig = mockBehandlingMedStatusOgVirkdato(
            BehandlingStatus.IVERKSATT,
            YearMonth.of(2000, Month.JANUARY)
        )
        val behandlingSen = mockBehandlingMedStatusOgVirkdato(
            BehandlingStatus.IVERKSATT,
            YearMonth.of(2022, Month.JANUARY)
        )
        val behandlingImellom = mockBehandlingMedStatusOgVirkdato(
            BehandlingStatus.IVERKSATT,
    ...
    fun `gir null hvis ingen iverksatte behandlinger`() {
        val behandlingAttestert = mockBehandlingMedStatusOgVirkdato(
            BehandlingStatus.ATTESTERT,
            YearMonth.of(2022, Month.APRIL)
        )
        val behandlingReturnert = mockBehandlingMedStatusOgVirkdato(
            BehandlingStatus.RETURNERT,
            YearMonth.of(2023, Month.MAY)
        )
        val behandlingUtenVirk = mockBehandlingMedStatusOgVirkdato(
    ...
    private fun mockBehandlingMedStatusOgVirkdato(
        ...
        return mockk<Behandling> {
            ...
            if (virkDato != null) {
                every { virkningstidspunkt } returns mockk<Virkningstidspunkt> {
