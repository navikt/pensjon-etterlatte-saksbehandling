...
import no.nav.etterlatte.brev.behandling.ManglerAvdoedBruktTilTrygdetid
...
internal fun barnepensjonBeregning(
    ...
    return BarnepensjonBeregning(
        ...
        trygdetid = trygdetid.map { it.toTrygdetidMedBeregningsmetode(anvendtMetode, metodeFraGrunnlag, avdoede) },
        erForeldreloes = erForeldreloes,
        bruktTrygdetid =
            trygdetid.find {
                it.ident == sisteBeregningsperiode.trygdetidForIdent
            }?.toTrygdetidMedBeregningsmetode(anvendtMetode, metodeFraGrunnlag, avdoede) ?: throw ManglerAvdoedBruktTilTrygdetid(),
    )
}
