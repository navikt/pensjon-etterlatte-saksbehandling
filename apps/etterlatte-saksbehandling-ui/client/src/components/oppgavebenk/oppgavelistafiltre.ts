...
export function sortrerFrist(retning: Retning, oppgaver: OppgaveDTO[]) {
  switch (retning) {
    case 'ascending':
...
export function filtrerOppgaver(
  ...
  const fristSortrert = sortrerFrist(fristRetning, fristFiltrert)

  return finnFnrIOppgaver(fnr, fristSortrert)
}
