...
import {DekrypterResponse} from "~shared/types/Krypter";
...
export const Person = () => {
  ...
  const { fnr } = useParams()
  const [fnrTilBruk, setFnrTilBruk]= useState('')
  ...
  const handleError = (error: ApiError) => {
    if (error.status === 400) {
      ...
      return <ApiErrorAlert>Feil oppsto ved henting av person med fødselsnummer {fnrTilBruk}</ApiErrorAlert>
    }
  }
  ...
  return (
      fnrTilBruk && fnrHarGyldigFormat(fnrTilBruk) ?
    <>
      ...
      {mapAllApiResult(
        ...
        (person) => visFaner(person)
      )}
          </> : <ApiErrorAlert>Fødselsnummeret {fnrTilBruk} har et ugyldig format (ikke 11 siffer)</ApiErrorAlert>
  )

  function visFaner(person: IPdlPersonNavnFoedsel) {
    return <Tabs value={fane} onChange={velgFane}>
      <Tabs.List>
