...
class KvitteringMottaker(
    ...
    private fun Utbetaling.kvitteringFeilmelding() =
        when (this.kvittering?.alvorlighetsgrad) {
            "00" -> null
            else -> "${this.kvittering?.kode} ${this.kvittering?.beskrivelse}"
        }

    private fun Oppdrag.haandterMottattKvittering(
        utbetaling: Utbetaling,
        oppdragXml: String,
