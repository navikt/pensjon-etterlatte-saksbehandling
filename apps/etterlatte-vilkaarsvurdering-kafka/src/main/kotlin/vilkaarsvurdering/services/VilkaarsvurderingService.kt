...
interface VilkaarsvurderingService {
    ...
    fun migrer(request: VilkaarsvurderingMigreringRequest): HttpResponse
}
...
class VilkaarsvurderingServiceImpl(private val vilkaarsvurderingKlient: HttpClient, private val url: String) :
    ...
    override fun migrer(request: VilkaarsvurderingMigreringRequest) = runBlocking {
        vilkaarsvurderingKlient.post(
            "$url/api/vilkaarsvurdering/migrering"
        ) {
            contentType(ContentType.Application.Json)
            setBody(request)
        }
