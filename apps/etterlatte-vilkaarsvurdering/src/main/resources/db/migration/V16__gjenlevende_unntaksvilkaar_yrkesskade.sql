DO $$
    DECLARE
        id_value uuid;
    BEGIN
        FOR id_value IN (SELECT DISTINCT vilkaar.id FROM vilkaar, delvilkaar WHERE vilkaar.id = delvilkaar.vilkaar_id AND delvilkaar.vilkaar_type = 'OMS_GJENLEVENDES_MEDLEMSKAP_UNNTAK_YRKESSKADE')
            LOOP
