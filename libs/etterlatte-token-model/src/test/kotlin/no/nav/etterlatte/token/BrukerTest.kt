...
internal class BrukerTest {
    ...
    fun `er maskin-til-maskin viss oid og sub er like`() {
        assertTrue(Bruker("a", Saksbehandler("b"), "c", "c").erMaskinTilMaskin())
    }

    @Test
    fun `er ikke maskin-til-maskin viss oid og sub er ulike`() {
        assertFalse(Bruker("a", Saksbehandler("b"), "c", "d").erMaskinTilMaskin())
    }

    @Test
    fun `er ikke maskin-til-maskin viss oid er null, men sub har verdi`() {
        assertFalse(Bruker("a", Saksbehandler("b"), null, "d").erMaskinTilMaskin())
    }

    @Test
    fun `er ikke maskin-til-maskin viss sub er null, men oid har verdi`() {
        assertFalse(Bruker("a", Saksbehandler("b"), "c", null).erMaskinTilMaskin())
    }

    @Test
    fun `er ikke maskin-til-maskin viss både oid og sub er null`() {
        assertFalse(Bruker("a", Saksbehandler("b"), null, null).erMaskinTilMaskin())
    }
}